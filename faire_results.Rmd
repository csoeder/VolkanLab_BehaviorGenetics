...
sampleBreakdown.gt<- data_sets.df.sparse  %>% group_by(experimental,rep) %>% summarise(`sample count`=n())  %>% gt() %>%  tab_options(row_group.background.color = "#FFEFDB")  %>% tab_header(title=paste("Table ",tbl_cnt, ". Breakdown of Samples within Experiment", sep = ""), subtitle= md("&nbsp;"))
...
tbl_cnt <- tbl_cnt + 1
paste("Table ",tbl_cnt, ". Size and Consolidation of Reference Genomes", sep = "")
...
png(height =  500, width = 800, filename = "results/figures/supp/Suppfig_ReadPeakStatCorr_byExp.png", sep="")
...
intraSamplePeakDistanceRawCollapsed

...
contrastedPeakStats_shared.df %>% filter(type == "avg" | type== "locii" | type=="locii_intersect" | type=="input" | type=="output" | type=="input_intersect" | type=="output_intersect") %>% unite(measure, type, "measure") %>% spread(key=measure, value=value) %>% group_by(experimental) %>% gt() %>% cols_label(locii_count = "locii", input_count = "input", output_count = "output", avg_input_width = "input", avg_output_width = "output", avg_input_signal = "input", avg_output_signal = "output", input_intersect_count="input", output_intersect_count="output", locii_intersect_count="locii", avg_distance = "w/ intersects", avg_non0dist = "w/o intersects", group = "" ) %>% tab_spanner(label = "number shared", columns =c("locii_count", "input_count", "output_count")) %>% tab_spanner(label = "avg width (bp)", columns =c("avg_input_width", "avg_output_width")) %>% tab_spanner(label = "avg signal", columns =c("avg_input_signal", "avg_output_signal"))  %>% tab_spanner(label = "number intersects", columns =c("input_intersect_count", "output_intersect_count", "locii_intersect_count")) %>% tab_spanner(label = "average distance (bp)", columns =c("avg_distance", "avg_non0dist"))

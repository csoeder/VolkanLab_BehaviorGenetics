...
#ggplot( ) + geom_freqpoly(data = calledPeaks.selfDistance.df %>% filter(chrom1 != "chrM") %>% mutate(process = "raw") , aes(x=abs(distance), color=rep), bins=50) + facet_grid(experimental ~ chrom1) + scale_x_log10() +theme_bw()+ theme(axis.text.x = element_text(angle = 90, hjust = 1)) + labs(title = "Distance Between Adjacent Peaks,\nby sample and replicate", x= "distance (bp)") + theme_bw()

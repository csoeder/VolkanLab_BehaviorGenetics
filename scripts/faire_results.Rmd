...
```
	...
	* Pessimistic: like weighted, but replicates with no contributing peaks are considered as having a peak of width 0 and signal 0 
	* Fine: (in development)
```
...
Since the inputs consist of one replicate each, their collapsed peaks are identical to their raw peaks. In the case of the outputs, with 1-3 replicates depending on experimental treatment and 
...
ggplot(collapsedPeaks.gath %>%  filter(value<20)) + geom_violin(data = inner_join(raw_called_peaks.signlXtract %>% filter(rep != "input") %>% select(sample, signal, rep, measure),data_sets.df %>% filter(subgroups != "all" & rep != "input") %>% select(name, subgroups, experimental) %>% rename(group=subgroups),by=c("sample"="name")) %>%  filter(signal<20), aes(x=measure, y=signal, fill=rep)) + geom_violin(aes(x=measure, y=value)) + facet_grid(group~experimental) + scale_y_log10()

hiPass.raw <- raw_called_peaks.signlXtract %>% filter(rep != "input") %>% mutate(high = signal > 20) %>% group_by(high) %>% summarize( count = n(), frac = count/sum(count))
hiPass.clps <- collapsedPeaks.gath  %>% mutate(high = value > 20) %>% group_by(high) %>% summarize( count = n(), frac = count/sum(count))
